...
const GET_AUDIO_FEATURES_SINGLE_ENDPOINT = `https://api.spotify.com/v1/audio-features`;
...
export const getSingleAudioFeatures = async (id) => {
    const { access_token } = await getAccessToken();
    //only handles up to 100 tracks

    return fetch(`${GET_AUDIO_FEATURES_SINGLE_ENDPOINT}/${id}`), {
        headers: {
            "Authorization": `Bearer ${access_token}`
        }
    }
